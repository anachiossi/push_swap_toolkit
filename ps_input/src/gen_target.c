...
/*   Updated: 2026/06/14 22:59:12 by anade-mo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_algorithm(t_gen *g)
{
	...
	while (g->level == -1)
	{
		...
		if (!line)
			return (0);
		if (!ft_strncmp(line, "adaptive\n", 9))
			g->level = 0;
		else if (!ft_strncmp(line, "simple\n", 7))
			g->level = 1;
		else if (!ft_strncmp(line, "medium\n", 7))
			g->level = 2;
		else if (!ft_strncmp(line, "complex\n", 8))
			g->level = 3;
		else
			ft_printf("Invalid: enter adaptive, simple, medium or complex\n");
		free(line);
	}
	return (1);
}
...
int	get_rank_or_value(t_gen *g)
{
	...
	while (g->use_rank == -1)
	{
		ft_printf("Print rank or value (random ints)?\n(rank / value)\n");
		line = get_next_line(0);
		if (!line)
			return (0);
		if (!ft_strncmp(line, "rank\n", 5))
			g->use_rank = 1;
		else if (!ft_strncmp(line, "value\n", 6))
			g->use_rank = 0;
		else
			ft_printf("Invalid: enter rank or value\n");
		free(line);
	}
	return (1);
}

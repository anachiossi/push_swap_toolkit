...
| [`ps_graph/`](ps_graph/README.md) | Executes a comprehensive sweep (runs push_swap 5 times for each size x disorder combination). Outputs a .txt with all the data. From that, some py scripts create graphs to compare your algorithms, calculate the empirical big O, audit if your adaptive mode chooses the best algorithm, and draw a heatmap of which algorithm wins at each case (size, disorder). |
